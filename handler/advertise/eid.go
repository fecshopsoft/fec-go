...
func EidList(c *gin.Context){
    ...
    // eid 搜索
    if eid != "" {
        q = q.Must(elastic.NewTermQuery("eid", eid))
    }
    // uv 范围搜索
    if uv_begin != "" || uv_end != "" {
    ...
    esAdvertiseEidTypeName :=  helper.GetEsAdvertiseEidTypeName()
    esIndexName := helper.GetEsIndexNameByType(esAdvertiseEidTypeName)
    ...
    log.Println(esAdvertiseEidTypeName)
    log.Println(page-1)
    log.Println(limit)
    log.Println(sort)
    search := client.Search().
        Index(esIndexName).        // search in index "twitter"
        Type(esAdvertiseEidTypeName).
        Query(q).
    ...
    */
    ...
    var ts []model.AdvertiseEidValue
    if searchResult.Hits.TotalHits > 0 {
        ...
        for _, hit := range searchResult.Hits.Hits {
            ...
            var advertiseEid model.AdvertiseEidValue
            err := json.Unmarshal(*hit.Source, &advertiseEid)
            if err != nil{
                c.AbortWithStatusJSON(http.StatusOK, util.BuildFailResult(err.Error()))
                return
            }
            ts = append(ts, advertiseEid)
        }
    }
...
// 得到 trend  info
func EidTrendInfo(c *gin.Context){
    eid := c.DefaultQuery("eid", "")
    if eid == ""{
        c.AbortWithStatusJSON(http.StatusOK, util.BuildFailResult("request get param eid can not empty"))
        return
    }
    ...
    if searchResult.Hits.TotalHits > 0 {
        ...
        for _, hit := range searchResult.Hits.Hits {
            ...
            serviceDateStr := advertiseEid.ServiceDateStr
            // pvTrend
            pvTrend[serviceDateStr] = advertiseEid.Pv
            // uvTrend
            uvTrend[serviceDateStr] = advertiseEid.Uv
            ipCountTrend[serviceDateStr] = advertiseEid.IpCount
            // staySecondsTrend
            staySecondsTrend[serviceDateStr] = advertiseEid.StaySeconds
            // staySecondsRateTrend
            staySecondsRateTrend[serviceDateStr] = advertiseEid.StaySecondsRate
            PvRateTrend[serviceDateStr] = advertiseEid.PvRate
            JumpOutCountTrend[serviceDateStr] = advertiseEid.JumpOutCount
            DropOutCountTrend[serviceDateStr] = advertiseEid.DropOutCount
            JumpOutRateTrend[serviceDateStr] = advertiseEid.JumpOutRate
            DropOutRateTrend[serviceDateStr] = advertiseEid.DropOutRate
            
            IsReturnTrend[serviceDateStr] = advertiseEid.IsReturn
            IsReturnRateTrend[serviceDateStr] = advertiseEid.IsReturnRate

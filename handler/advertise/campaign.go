...
func CampaignList(c *gin.Context){
    ...
    esAdvertiseCampaignTypeName :=  helper.GetEsAdvertiseCampaignTypeName()
    esIndexName := helper.GetEsIndexNameByType(esAdvertiseCampaignTypeName)
    ...
    log.Println(esAdvertiseCampaignTypeName)
    log.Println(page-1)
    log.Println(limit)
    log.Println(sort)
    search := client.Search().
        Index(esIndexName).        // search in index "twitter"
        Type(esAdvertiseCampaignTypeName).
        Query(q).
    ...
    */
    ...
    var ts []model.AdvertiseCampaignValue
    ...
    if searchResult.Hits.TotalHits > 0 {
        ...
        for _, hit := range searchResult.Hits.Hits {
            ...
            var advertiseCampaign model.AdvertiseCampaignValue
            err := json.Unmarshal(*hit.Source, &advertiseCampaign)
            if err != nil{
                log.Println("###1")
                c.AbortWithStatusJSON(http.StatusOK, util.BuildFailResult(err.Error()))
                return
            }
            ts = append(ts, advertiseCampaign)
            
            fecDesign := advertiseCampaign.FecDesign
            ...
            fecContent := advertiseCampaign.FecContent
            ...
            fecMarketGroup := advertiseCampaign.FecMarketGroup
...
// 得到 trend  info
func CampaignTrendInfo(c *gin.Context){
    fec_campaign := c.DefaultQuery("fec_campaign", "")
    if fec_campaign == ""{
        c.AbortWithStatusJSON(http.StatusOK, util.BuildFailResult("request get param fec_campaign can not empty"))
        return
    }
    ...
    if searchResult.Hits.TotalHits > 0 {
        ...
        for _, hit := range searchResult.Hits.Hits {
            ...
            serviceDateStr := advertiseCampaign.ServiceDateStr
            
            RegisterCountTrend[serviceDateStr] = advertiseCampaign.RegisterCount
            LoginCountTrend[serviceDateStr] = advertiseCampaign.LoginCount
            CategoryCountTrend[serviceDateStr] = advertiseCampaign.CategoryCount
            SkuCountTrend[serviceDateStr] = advertiseCampaign.SkuCount
            SearchCountTrend[serviceDateStr] = advertiseCampaign.SearchCount
            success_order_c_success_uv_rateTrend[serviceDateStr] = advertiseCampaign.SuccessOrderCSuccessUvRate
            success_order_c_all_uv_rateTrend[serviceDateStr] = advertiseCampaign.SuccessOrderCAllUvRate
            // pvTrend
            pvTrend[serviceDateStr] = advertiseCampaign.Pv
            // uvTrend
            uvTrend[serviceDateStr] = advertiseCampaign.Uv
            ipCountTrend[serviceDateStr] = advertiseCampaign.IpCount
            // staySecondsTrend
            staySecondsTrend[serviceDateStr] = advertiseCampaign.StaySeconds
            // staySecondsRateTrend
            staySecondsRateTrend[serviceDateStr] = advertiseCampaign.StaySecondsRate
            PvRateTrend[serviceDateStr] = advertiseCampaign.PvRate
            JumpOutCountTrend[serviceDateStr] = advertiseCampaign.JumpOutCount
            DropOutCountTrend[serviceDateStr] = advertiseCampaign.DropOutCount
            JumpOutRateTrend[serviceDateStr] = advertiseCampaign.JumpOutRate
            DropOutRateTrend[serviceDateStr] = advertiseCampaign.DropOutRate
            CartCountTrend[serviceDateStr] = advertiseCampaign.CartCount
            OrderCountTrend[serviceDateStr] = advertiseCampaign.OrderCount
            SuccessOrderCountTrend[serviceDateStr] = advertiseCampaign.SuccessOrderCount
            SuccessOrderNoCountTrend[serviceDateStr] = advertiseCampaign.SuccessOrderNoCount
            OrderNoCountTrend[serviceDateStr] = advertiseCampaign.OrderNoCount
            OrderPaymentRateTrend[serviceDateStr] = advertiseCampaign.OrderPaymentRate
            OrderAmountTrend[serviceDateStr] = advertiseCampaign.OrderAmount
            SuccessOrderAmountTrend[serviceDateStr] = advertiseCampaign.SuccessOrderAmount
            
            IsReturnTrend[serviceDateStr] = advertiseCampaign.IsReturn
            IsReturnRateTrend[serviceDateStr] = advertiseCampaign.IsReturnRate
            SkuSaleRateTrend[serviceDateStr] = advertiseCampaign.SkuSaleRate
        }
    }
    // 生成返回结果
    result := util.BuildSuccessResult(gin.H{

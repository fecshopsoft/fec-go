...
func MediumList(c *gin.Context){
    ...
    esAdvertiseMediumTypeName :=  helper.GetEsAdvertiseMediumTypeName()
    esIndexName := helper.GetEsIndexNameByType(esAdvertiseMediumTypeName)
    ...
    log.Println(esAdvertiseMediumTypeName)
    log.Println(page-1)
    log.Println(limit)
    log.Println(sort)
    search := client.Search().
        Index(esIndexName).        // search in index "twitter"
        Type(esAdvertiseMediumTypeName).
        Query(q).
    ...
    */
    ...
    var ts []model.AdvertiseMediumValue
    ...
    if searchResult.Hits.TotalHits > 0 {
        ...
        for _, hit := range searchResult.Hits.Hits {
            ...
            var advertiseMedium model.AdvertiseMediumValue
            err := json.Unmarshal(*hit.Source, &advertiseMedium)
            if err != nil{
                log.Println("###1")
                c.AbortWithStatusJSON(http.StatusOK, util.BuildFailResult(err.Error()))
                return
            }
            ts = append(ts, advertiseMedium)
            
            fecDesign := advertiseMedium.FecDesign
            ...
            fecContent := advertiseMedium.FecContent
            ...
            fecMarketGroup := advertiseMedium.FecMarketGroup
...
// 得到 trend  info
func MediumTrendInfo(c *gin.Context){
    fec_medium := c.DefaultQuery("fec_medium", "")
    if fec_medium == ""{
        c.AbortWithStatusJSON(http.StatusOK, util.BuildFailResult("request get param fec_medium can not empty"))
        return
    }
    ...
    if searchResult.Hits.TotalHits > 0 {
        ...
        for _, hit := range searchResult.Hits.Hits {
            ...
            serviceDateStr := advertiseMedium.ServiceDateStr
            
            RegisterCountTrend[serviceDateStr] = advertiseMedium.RegisterCount
            LoginCountTrend[serviceDateStr] = advertiseMedium.LoginCount
            CategoryCountTrend[serviceDateStr] = advertiseMedium.CategoryCount
            SkuCountTrend[serviceDateStr] = advertiseMedium.SkuCount
            SearchCountTrend[serviceDateStr] = advertiseMedium.SearchCount
            success_order_c_success_uv_rateTrend[serviceDateStr] = advertiseMedium.SuccessOrderCSuccessUvRate
            success_order_c_all_uv_rateTrend[serviceDateStr] = advertiseMedium.SuccessOrderCAllUvRate
            // pvTrend
            pvTrend[serviceDateStr] = advertiseMedium.Pv
            // uvTrend
            uvTrend[serviceDateStr] = advertiseMedium.Uv
            ipCountTrend[serviceDateStr] = advertiseMedium.IpCount
            // staySecondsTrend
            staySecondsTrend[serviceDateStr] = advertiseMedium.StaySeconds
            // staySecondsRateTrend
            staySecondsRateTrend[serviceDateStr] = advertiseMedium.StaySecondsRate
            PvRateTrend[serviceDateStr] = advertiseMedium.PvRate
            JumpOutCountTrend[serviceDateStr] = advertiseMedium.JumpOutCount
            DropOutCountTrend[serviceDateStr] = advertiseMedium.DropOutCount
            JumpOutRateTrend[serviceDateStr] = advertiseMedium.JumpOutRate
            DropOutRateTrend[serviceDateStr] = advertiseMedium.DropOutRate
            CartCountTrend[serviceDateStr] = advertiseMedium.CartCount
            OrderCountTrend[serviceDateStr] = advertiseMedium.OrderCount
            SuccessOrderCountTrend[serviceDateStr] = advertiseMedium.SuccessOrderCount
            SuccessOrderNoCountTrend[serviceDateStr] = advertiseMedium.SuccessOrderNoCount
            OrderNoCountTrend[serviceDateStr] = advertiseMedium.OrderNoCount
            OrderPaymentRateTrend[serviceDateStr] = advertiseMedium.OrderPaymentRate
            OrderAmountTrend[serviceDateStr] = advertiseMedium.OrderAmount
            SuccessOrderAmountTrend[serviceDateStr] = advertiseMedium.SuccessOrderAmount
            
            IsReturnTrend[serviceDateStr] = advertiseMedium.IsReturn
            IsReturnRateTrend[serviceDateStr] = advertiseMedium.IsReturnRate
            SkuSaleRateTrend[serviceDateStr] = advertiseMedium.SkuSaleRate
        }
    }
    // 生成返回结果
    result := util.BuildSuccessResult(gin.H{

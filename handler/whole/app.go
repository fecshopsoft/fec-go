...
func AppList(c *gin.Context){
    ...
    // app 搜索
    if app != "" {
        q = q.Must(elastic.NewTermQuery("app", app))
    }
    // uv 范围搜索
    if uv_begin != "" || uv_end != "" {
    ...
    */
    ...
    if searchResult.Hits.TotalHits > 0 {
        ...
        for _, hit := range searchResult.Hits.Hits {
            ...
            var WholeApp model.WholeAppValue
            err := json.Unmarshal(*hit.Source, &WholeApp)
            if err != nil{
                c.AbortWithStatusJSON(http.StatusOK, util.BuildFailResult(err.Error()))
                return
            }
            ts = append(ts, WholeApp)
        }
    }
...
// 得到 trend  info
func AppTrendInfo(c *gin.Context){
    app := c.DefaultQuery("app", "")
    if app == ""{
        c.AbortWithStatusJSON(http.StatusOK, util.BuildFailResult("request get param app can not empty"))
        return
    }
    ...
    if searchResult.Hits.TotalHits > 0 {
        ...
        for _, hit := range searchResult.Hits.Hits {
            ...
            serviceDateStr := WholeApp.ServiceDateStr
            // pvTrend
            pvTrend[serviceDateStr] = WholeApp.Pv
            // uvTrend
            uvTrend[serviceDateStr] = WholeApp.Uv
            ipCountTrend[serviceDateStr] = WholeApp.IpCount
            // staySecondsTrend
            staySecondsTrend[serviceDateStr] = WholeApp.StaySeconds
            // staySecondsRateTrend
            staySecondsRateTrend[serviceDateStr] = WholeApp.StaySecondsRate
            PvRateTrend[serviceDateStr] = WholeApp.PvRate
            JumpOutCountTrend[serviceDateStr] = WholeApp.JumpOutCount
            DropOutCountTrend[serviceDateStr] = WholeApp.DropOutCount
            JumpOutRateTrend[serviceDateStr] = WholeApp.JumpOutRate
            DropOutRateTrend[serviceDateStr] = WholeApp.DropOutRate
            CartCountTrend[serviceDateStr] = WholeApp.CartCount
            OrderCountTrend[serviceDateStr] = WholeApp.OrderCount
            SuccessOrderCountTrend[serviceDateStr] = WholeApp.SuccessOrderCount
            SuccessOrderNoCountTrend[serviceDateStr] = WholeApp.SuccessOrderNoCount
            OrderNoCountTrend[serviceDateStr] = WholeApp.OrderNoCount
            OrderPaymentRateTrend[serviceDateStr] = WholeApp.OrderPaymentRate
            OrderAmountTrend[serviceDateStr] = WholeApp.OrderAmount
            SuccessOrderAmountTrend[serviceDateStr] = WholeApp.SuccessOrderAmount
            
            IsReturnTrend[serviceDateStr] = WholeApp.IsReturn
            IsReturnRateTrend[serviceDateStr] = WholeApp.IsReturnRate
            SkuSaleRateTrend[serviceDateStr] = WholeApp.SkuSaleRate
        }
    }
    // 生成返回结果
    result := util.BuildSuccessResult(gin.H{

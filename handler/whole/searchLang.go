...
func SearchLangList(c *gin.Context){
    ...
    // language 搜索
    if language != "" {
        q = q.Must(elastic.NewTermQuery("language", language))
    }
    // uv 范围搜索
    if uv_begin != "" || uv_end != "" {
   ...
   ////// chosen_own_id,  selectOwnIds, 
    ...
    search := client.Search().
        ...
        Type(esWholeSearchLangTypeName).
        Query(q).
    ...
    */
    ...
    var ts []model.WholeSearchLangValue
    if searchResult.Hits.TotalHits > 0 {
        ...
        for _, hit := range searchResult.Hits.Hits {
            ...
            var wholeSearchLang model.WholeSearchLangValue
            err := json.Unmarshal(*hit.Source, &wholeSearchLang)
            if err != nil{
                c.AbortWithStatusJSON(http.StatusOK, util.BuildFailResult(err.Error()))
                return
            }
            ts = append(ts, wholeSearchLang)
        }
    }
...
// 得到 trend  info
func SearchLangTrendInfo(c *gin.Context){
    search_text := c.DefaultQuery("search_text", "")
    if search_text == ""{
    ...
    language := c.DefaultQuery("language", "")
    if language == ""{
        c.AbortWithStatusJSON(http.StatusOK, util.BuildFailResult("request get param language can not empty"))
        return
    }
    ...
    if searchResult.Hits.TotalHits > 0 {
        ...
        for _, hit := range searchResult.Hits.Hits {
            ...
            serviceDateStr := wholeSearchLang.ServiceDateStr
            // pvTrend
            pvTrend[serviceDateStr] = wholeSearchLang.Pv
            // uvTrend
            uvTrend[serviceDateStr] = wholeSearchLang.Uv
            ipCountTrend[serviceDateStr] = wholeSearchLang.IpCount
            // staySecondsTrend
            staySecondsTrend[serviceDateStr] = wholeSearchLang.StaySeconds
            // staySecondsRateTrend
            staySecondsRateTrend[serviceDateStr] = wholeSearchLang.StaySecondsRate
            PvRateTrend[serviceDateStr] = wholeSearchLang.PvRate
            JumpOutCountTrend[serviceDateStr] = wholeSearchLang.JumpOutCount
            DropOutCountTrend[serviceDateStr] = wholeSearchLang.DropOutCount
            JumpOutRateTrend[serviceDateStr] = wholeSearchLang.JumpOutRate
            DropOutRateTrend[serviceDateStr] = wholeSearchLang.DropOutRate
            
            IsReturnTrend[serviceDateStr] = wholeSearchLang.IsReturn
            IsReturnRateTrend[serviceDateStr] = wholeSearchLang.IsReturnRate
            
            SearchSkuClickTrend[serviceDateStr] = wholeSearchLang.SearchSkuClick
            SearchLoginEmailTrend[serviceDateStr] = wholeSearchLang.SearchLoginEmail
            SearchSkuCartTrend[serviceDateStr] = wholeSearchLang.SearchSkuCart
            SearchSkuOrderTrend[serviceDateStr] = wholeSearchLang.SearchSkuOrder
            SearchSkuOrderSuccessTrend[serviceDateStr] = wholeSearchLang.SearchSkuOrderSuccess
            SearchQtyTrend[serviceDateStr] = wholeSearchLang.SearchQty
            
            SearchSkuClickRateTrend[serviceDateStr] = wholeSearchLang.SearchSkuClickRate
            SearchSaleRateTrend[serviceDateStr] = wholeSearchLang.SearchSaleRate
        }
    }
    // 生成返回结果
    result := util.BuildSuccessResult(gin.H{

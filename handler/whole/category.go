...
func CategoryList(c *gin.Context){
    ...
    category, _ = netUrl.QueryUnescape(category)
    ...
    // category 搜索
    if category != "" {
        q = q.Must(elastic.NewTermQuery("category", category))
    }
    // uv 范围搜索
    if uv_begin != "" || uv_end != "" {
    ...
    */
    ...
    var ts []model.WholeCategoryValue
    if searchResult.Hits.TotalHits > 0 {
        ...
        for _, hit := range searchResult.Hits.Hits {
            ...
            var WholeCategory model.WholeCategoryValue
            err := json.Unmarshal(*hit.Source, &WholeCategory)
            if err != nil{
                c.AbortWithStatusJSON(http.StatusOK, util.BuildFailResult(err.Error()))
                return
            }
            ts = append(ts, WholeCategory)
        }
    }
...
// 得到 trend  info
func CategoryTrendInfo(c *gin.Context){
    category := c.DefaultQuery("category", "")
    category, _ = netUrl.QueryUnescape(category)
    if category == ""{
        c.AbortWithStatusJSON(http.StatusOK, util.BuildFailResult("request get param category can not empty"))
        return
    }
    ...
    if searchResult.Hits.TotalHits > 0 {
        ...
        for _, hit := range searchResult.Hits.Hits {
            ...
            serviceDateStr := WholeCategory.ServiceDateStr
            // pvTrend
            pvTrend[serviceDateStr] = WholeCategory.Pv
            // uvTrend
            uvTrend[serviceDateStr] = WholeCategory.Uv
            ipCountTrend[serviceDateStr] = WholeCategory.IpCount
            // staySecondsTrend
            staySecondsTrend[serviceDateStr] = WholeCategory.StaySeconds
            // staySecondsRateTrend
            staySecondsRateTrend[serviceDateStr] = WholeCategory.StaySecondsRate
            PvRateTrend[serviceDateStr] = WholeCategory.PvRate
            JumpOutCountTrend[serviceDateStr] = WholeCategory.JumpOutCount
            DropOutCountTrend[serviceDateStr] = WholeCategory.DropOutCount
            JumpOutRateTrend[serviceDateStr] = WholeCategory.JumpOutRate
            DropOutRateTrend[serviceDateStr] = WholeCategory.DropOutRate
            
            IsReturnTrend[serviceDateStr] = WholeCategory.IsReturn
            IsReturnRateTrend[serviceDateStr] = WholeCategory.IsReturnRate

...
func FirstUrlList(c *gin.Context){
    ...
    */
    ...
    var ts []model.WholeFirstUrlValue
    if searchResult.Hits.TotalHits > 0 {
        ...
        for _, hit := range searchResult.Hits.Hits {
            ...
            var WholeFirstUrl model.WholeFirstUrlValue
            err := json.Unmarshal(*hit.Source, &WholeFirstUrl)
            if err != nil{
                c.AbortWithStatusJSON(http.StatusOK, util.BuildFailResult(err.Error()))
                return
            }
            ts = append(ts, WholeFirstUrl)
        }
    }
...
// 得到 trend  info
func FirstUrlTrendInfo(c *gin.Context){
    url := c.DefaultQuery("url", "")
    url, _ = netUrl.QueryUnescape(url)
    if url == ""{
    ...
    if searchResult.Hits.TotalHits > 0 {
        ...
        for _, hit := range searchResult.Hits.Hits {
            ...
            serviceDateStr := WholeFirstUrl.ServiceDateStr
            // pvTrend
            pvTrend[serviceDateStr] = WholeFirstUrl.Pv
            // uvTrend
            uvTrend[serviceDateStr] = WholeFirstUrl.Uv
            ipCountTrend[serviceDateStr] = WholeFirstUrl.IpCount
            // staySecondsTrend
            staySecondsTrend[serviceDateStr] = WholeFirstUrl.StaySeconds
            // staySecondsRateTrend
            staySecondsRateTrend[serviceDateStr] = WholeFirstUrl.StaySecondsRate
            PvRateTrend[serviceDateStr] = WholeFirstUrl.PvRate
            JumpOutCountTrend[serviceDateStr] = WholeFirstUrl.JumpOutCount
            DropOutCountTrend[serviceDateStr] = WholeFirstUrl.DropOutCount
            JumpOutRateTrend[serviceDateStr] = WholeFirstUrl.JumpOutRate
            DropOutRateTrend[serviceDateStr] = WholeFirstUrl.DropOutRate
            
            IsReturnTrend[serviceDateStr] = WholeFirstUrl.IsReturn
            IsReturnRateTrend[serviceDateStr] = WholeFirstUrl.IsReturnRate

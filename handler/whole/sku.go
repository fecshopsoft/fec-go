    ...
    */
    ...
    if searchResult.Hits.TotalHits > 0 {
        ...
        for _, hit := range searchResult.Hits.Hits {
            ...
            var wholeSku model.WholeSkuValue
            err := json.Unmarshal(*hit.Source, &wholeSku)
            if err != nil{
                c.AbortWithStatusJSON(http.StatusOK, util.BuildFailResult(err.Error()))
                return
            }
...
// 得到 trend  info
func SkuTrendInfo(c *gin.Context){
    sku := c.DefaultQuery("sku", "")
    if sku == ""{
    ...
    if searchResult.Hits.TotalHits > 0 {
        ...
        for _, hit := range searchResult.Hits.Hits {
            ...
            serviceDateStr := wholeSku.ServiceDateStr
            // pvTrend
            pvTrend[serviceDateStr] = wholeSku.Pv
            // uvTrend
            uvTrend[serviceDateStr] = wholeSku.Uv
            ipCountTrend[serviceDateStr] = wholeSku.IpCount
            // staySecondsTrend
            staySecondsTrend[serviceDateStr] = wholeSku.StaySeconds
            // staySecondsRateTrend
            staySecondsRateTrend[serviceDateStr] = wholeSku.StaySecondsRate
            PvRateTrend[serviceDateStr] = wholeSku.PvRate
            JumpOutCountTrend[serviceDateStr] = wholeSku.JumpOutCount
            DropOutCountTrend[serviceDateStr] = wholeSku.DropOutCount
            JumpOutRateTrend[serviceDateStr] = wholeSku.JumpOutRate
            DropOutRateTrend[serviceDateStr] = wholeSku.DropOutRate
            CartCountTrend[serviceDateStr] = wholeSku.CartCount
            OrderCountTrend[serviceDateStr] = wholeSku.OrderCount
            SuccessOrderCountTrend[serviceDateStr] = wholeSku.SuccessOrderCount
            SuccessOrderNoCountTrend[serviceDateStr] = wholeSku.SuccessOrderNoCount
            OrderNoCountTrend[serviceDateStr] = wholeSku.OrderNoCount
            OrderPaymentRateTrend[serviceDateStr] = wholeSku.OrderPaymentRate
            OrderAmountTrend[serviceDateStr] = wholeSku.OrderAmount
            SuccessOrderAmountTrend[serviceDateStr] = wholeSku.SuccessOrderAmount
            
            IsReturnTrend[serviceDateStr] = wholeSku.IsReturn
            IsReturnRateTrend[serviceDateStr] = wholeSku.IsReturnRate
            SkuSaleRateTrend[serviceDateStr] = wholeSku.SkuSaleRate
        }
    }
    // 生成返回结果
    result := util.BuildSuccessResult(gin.H{

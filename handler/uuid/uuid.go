...
func UuidList(c *gin.Context){
    ...
    // customer_id 搜索
    if customer_id != "" {
        q = q.Must(elastic.NewTermQuery("id", customer_id))
    }
...
func UuidTrendInfo(c *gin.Context){
    ...
    if searchResult.Hits.TotalHits > 0 {
        ...
        for _, hit := range searchResult.Hits.Hits {
            ...
            RegisterCountTrend[serviceDateStr] = customerUuid.RegisterCount
            LoginCountTrend[serviceDateStr] = customerUuid.LoginCount
            CategoryCountTrend[serviceDateStr] = customerUuid.CategoryCount
            SkuCountTrend[serviceDateStr] = customerUuid.SkuCount
            SearchCountTrend[serviceDateStr] = customerUuid.SearchCount
            success_order_c_success_uv_rateTrend[serviceDateStr] = customerUuid.SuccessOrderCSuccessUvRate
            success_order_c_all_uv_rateTrend[serviceDateStr] = customerUuid.SuccessOrderCAllUvRate
            // pvTrend
            pvTrend[serviceDateStr] = customerUuid.Pv
            // uvTrend
            uvTrend[serviceDateStr] = customerUuid.Uv
            ipCountTrend[serviceDateStr] = customerUuid.IpCount
            // staySecondsTrend
            staySecondsTrend[serviceDateStr] = customerUuid.StaySeconds
            // staySecondsRateTrend
            staySecondsRateTrend[serviceDateStr] = customerUuid.StaySecondsRate
            PvRateTrend[serviceDateStr] = customerUuid.PvRate
            JumpOutCountTrend[serviceDateStr] = customerUuid.JumpOutCount
            DropOutCountTrend[serviceDateStr] = customerUuid.DropOutCount
            JumpOutRateTrend[serviceDateStr] = customerUuid.JumpOutRate
            DropOutRateTrend[serviceDateStr] = customerUuid.DropOutRate
            CartCountTrend[serviceDateStr] = customerUuid.CartCount
            OrderCountTrend[serviceDateStr] = customerUuid.OrderCount
            SuccessOrderCountTrend[serviceDateStr] = customerUuid.SuccessOrderCount
            SuccessOrderNoCountTrend[serviceDateStr] = customerUuid.SuccessOrderNoCount
            OrderNoCountTrend[serviceDateStr] = customerUuid.OrderNoCount
            OrderPaymentRateTrend[serviceDateStr] = customerUuid.OrderPaymentRate
            OrderAmountTrend[serviceDateStr] = customerUuid.OrderAmount
            SuccessOrderAmountTrend[serviceDateStr] = customerUuid.SuccessOrderAmount
            
            IsReturnTrend[serviceDateStr] = customerUuid.IsReturn
            IsReturnRateTrend[serviceDateStr] = customerUuid.IsReturnRate
            SkuSaleRateTrend[serviceDateStr] = customerUuid.SkuSaleRate
        }
    }
    // 生成返回结果
    result := util.BuildSuccessResult(gin.H{

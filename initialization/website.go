...
type SiteInfos map[string]string  // string中存储的是websiteUid
...
func InitWebsiteInfo() error{
	...
	websites, err := commonHandler.GetAllActiveWebsites()
	if err != nil {
		return err
	}
	// 得到激活状态的website信息。供接收数据部分使用
	//var websiteIds []string
	for j:=0; j<len(websites); j++ {
		website := websites[j]
		siteUid := website.SiteUid
		WebsiteInfos[siteUid]  = siteUid
	...
	// 得到激活状态的所有的用户
	customers, err:= customerHandler.GetActiveCustomers()
    if err != nil {
        return err
    }

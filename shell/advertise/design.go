...
// MarketGroup 统计计算部分
func DesignMapReduct(dbName string, collName string, outCollName string, website_id string) error {
    var err error
    mapStr := `
        function() {  
            ...
            if (fec_design) {
                ...
                if(fec_design){
                    ...
                    emit(fec_design+"_"+service_date_str+"_"+website_id,{
                        fid: fid,
    ...
    esAdvertiseDesignTypeMapping := helper.GetEsAdvertiseDesignTypeMapping()
    ...
    //}
    // 初始化mapping
    err = esdb.InitMapping(esIndexName, esAdvertiseDesignTypeName, esAdvertiseDesignTypeMapping)
    if err != nil {
        return err
    }
    // 同步mongo数据到ES
    ...
    for i:=0; i<pageNum; i++ {
        err = mongodb.MDC(dbName, outCollName, func(coll *mgo.Collection) error {
            var err error
            var advertiseDesigns []model.AdvertiseDesign
            coll.Find(nil).Skip(i*pageNum).Limit(numPerPage).All(&advertiseDesigns)
            log.Println("advertiseDesigns length:")
            log.Println(len(advertiseDesigns))
            
            /* 这个代码是upsert单行数据
            for j:=0; j<len(advertiseDesigns); j++ {
                AdvertiseDesign := advertiseDesigns[j]
                advertiseDesignValue := advertiseDesign.Value
                // advertiseDesignValue.Devide = nil
                // advertiseDesignValue.CountryCode = nil
                ///advertiseDesignValue.Operate = nil
                log.Println("ID_:" + advertiseDesign.Id_)
                advertiseDesignValue.Id = advertiseDesign.Id_
                err := esdb.UpsertType(esIndexName, esAdvertiseDesignTypeName, advertiseDesign.Id_, advertiseDesignValue)
            ...
            */
            if len(advertiseDesigns) > 0 {
                ...
                for j:=0; j<len(advertiseDesigns); j++ {
                    ...
                    log.Println(advertiseDesign.Id_)
                    log.Println(advertiseDesignValue)
                    req := esdb.BulkUpsertTypeDoc(esIndexName, esAdvertiseDesignTypeName, advertiseDesign.Id_, advertiseDesignValue)

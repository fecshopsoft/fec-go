...
// MarketGroup 统计计算部分
func MarketGroupMapReduct(dbName string, collName string, outCollName string, website_id string) error {
    var err error
    mapStr := `
        function() {  
            ...
            if (fec_market_group) {
                ...
                if(fec_market_group){
                    ...
                    emit(fec_market_group+"_"+service_date_str+"_"+website_id,{
                        fid: fid,
    ...
    esAdvertiseMarketGroupTypeMapping := helper.GetEsAdvertiseMarketGroupTypeMapping()
    ...
    //}
    // 初始化mapping
    err = esdb.InitMapping(esIndexName, esAdvertiseMarketGroupTypeName, esAdvertiseMarketGroupTypeMapping)
    if err != nil {
        return err
    }
    // 同步mongo数据到ES
    ...
    for i:=0; i<pageNum; i++ {
        err = mongodb.MDC(dbName, outCollName, func(coll *mgo.Collection) error {
            var err error
            var advertiseMarketGroups []model.AdvertiseMarketGroup
            coll.Find(nil).Skip(i*pageNum).Limit(numPerPage).All(&advertiseMarketGroups)
            log.Println("advertiseMarketGroups length:")
            log.Println(len(advertiseMarketGroups))
            
            /* 这个代码是upsert单行数据
            for j:=0; j<len(advertiseMarketGroups); j++ {
                AdvertiseMarketGroup := advertiseMarketGroups[j]
                advertiseMarketGroupValue := advertiseMarketGroup.Value
                // advertiseMarketGroupValue.Devide = nil
                // advertiseMarketGroupValue.CountryCode = nil
                ///advertiseMarketGroupValue.Operate = nil
                log.Println("ID_:" + advertiseMarketGroup.Id_)
                advertiseMarketGroupValue.Id = advertiseMarketGroup.Id_
                err := esdb.UpsertType(esIndexName, esAdvertiseMarketGroupTypeName, advertiseMarketGroup.Id_, advertiseMarketGroupValue)
            ...
            */
            if len(advertiseMarketGroups) > 0 {
                ...
                for j:=0; j<len(advertiseMarketGroups); j++ {
                    ...
                    log.Println(advertiseMarketGroup.Id_)
                    log.Println(advertiseMarketGroupValue)
                    req := esdb.BulkUpsertTypeDoc(esIndexName, esAdvertiseMarketGroupTypeName, advertiseMarketGroup.Id_, advertiseMarketGroupValue)

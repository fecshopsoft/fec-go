...
// Medium 统计计算部分
func MediumMapReduct(dbName string, collName string, outCollName string, website_id string) error {
    var err error
    mapStr := `
        function() {  
            ...
            if (fec_medium) {
                ...
                if(fec_medium){
                    ...
                    emit(fec_medium+"_"+service_date_str+"_"+website_id,{
                        fid: fid,
    ...
    esAdvertiseMediumTypeMapping := helper.GetEsAdvertiseMediumTypeMapping()
    ...
    //}
    // 初始化mapping
    err = esdb.InitMapping(esIndexName, esAdvertiseMediumTypeName, esAdvertiseMediumTypeMapping)
    if err != nil {
        return err
    }
    // 同步mongo数据到ES
    ...
    for i:=0; i<pageNum; i++ {
        err = mongodb.MDC(dbName, outCollName, func(coll *mgo.Collection) error {
            var err error
            var advertiseMediums []model.AdvertiseMedium
            coll.Find(nil).Skip(i*pageNum).Limit(numPerPage).All(&advertiseMediums)
            log.Println("advertiseMediums length:")
            log.Println(len(advertiseMediums))
            
            /* 这个代码是upsert单行数据
            for j:=0; j<len(advertiseMediums); j++ {
                AdvertiseMedium := advertiseMediums[j]
                advertiseMediumValue := advertiseMedium.Value
                // advertiseMediumValue.Devide = nil
                // advertiseMediumValue.CountryCode = nil
                ///advertiseMediumValue.Operate = nil
                log.Println("ID_:" + advertiseMedium.Id_)
                advertiseMediumValue.Id = advertiseMedium.Id_
                err := esdb.UpsertType(esIndexName, esAdvertiseMediumTypeName, advertiseMedium.Id_, advertiseMediumValue)
            ...
            */
            if len(advertiseMediums) > 0 {
                ...
                for j:=0; j<len(advertiseMediums); j++ {
                    ...
                    log.Println(advertiseMedium.Id_)
                    log.Println(advertiseMediumValue)
                    req := esdb.BulkUpsertTypeDoc(esIndexName, esAdvertiseMediumTypeName, advertiseMedium.Id_, advertiseMediumValue)

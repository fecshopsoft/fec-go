...
// Eid 统计计算部分
func EidMapReduct(dbName string, collName string, outCollName string, website_id string) error {
    var err error
    
    mapStr := `
        function() {  
            website_id = "` + website_id + `";
            eid = this.eid ? this.eid : null;
            if (eid) {
                ...
                if (eid) {
                    emit(eid+"_"+service_date_str+"_"+website_id,{
                        eid: eid,
    ...
    esAdvertiseEidTypeMapping := helper.GetEsAdvertiseEidTypeMapping()
    ...
    //}
    // 初始化mapping
    err = esdb.InitMapping(esIndexName, esAdvertiseEidTypeName, esAdvertiseEidTypeMapping)
    if err != nil {
        return err
    }
    // 同步mongo数据到ES
    ...
    for i:=0; i<pageNum; i++ {
        err = mongodb.MDC(dbName, outCollName, func(coll *mgo.Collection) error {
            var err error
            var advertiseEids []model.AdvertiseEid
            coll.Find(nil).Skip(i*pageNum).Limit(numPerPage).All(&advertiseEids)
            log.Println("advertiseEids length:")
            log.Println(len(advertiseEids))
            
            /* 这个代码是upsert单行数据
            for j:=0; j<len(advertiseEids); j++ {
                wholeBrowser := advertiseEids[j]
                ...
                err := esdb.UpsertType(esIndexName, esAdvertiseEidTypeName, wholeBrowser.Id_, wholeBrowserValue)
            ...
            */
            if len(advertiseEids) > 0 {
                ...
                for j:=0; j<len(advertiseEids); j++ {
                    advertiseEid := advertiseEids[j]
                    advertiseEidValue := advertiseEid.Value
                    advertiseEidValue.Id = advertiseEid.Id_
                    log.Println("888")
                    log.Println(esIndexName)
                    log.Println(esAdvertiseEidTypeName)
                    log.Println(advertiseEid.Id_)
                    log.Println(advertiseEidValue)
                    req := esdb.BulkUpsertTypeDoc(esIndexName, esAdvertiseEidTypeName, advertiseEid.Id_, advertiseEidValue)

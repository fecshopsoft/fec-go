...
// Content 统计计算部分
func ContentMapReduct(dbName string, collName string, outCollName string, website_id string) error {
    var err error
    mapStr := `
        function() {  
            ...
            if (fec_content) {
                ...
                if(this.fec_content){
                    ...
                    emit(this.fec_content+"_"+service_date_str+"_"+website_id,{
                        fid: fid,
    ...
    esAdvertiseContentTypeMapping := helper.GetEsAdvertiseContentTypeMapping()
    ...
    //}
    // 初始化mapping
    err = esdb.InitMapping(esIndexName, esAdvertiseContentTypeName, esAdvertiseContentTypeMapping)
    if err != nil {
        return err
    }
    // 同步mongo数据到ES
    ...
    for i:=0; i<pageNum; i++ {
        err = mongodb.MDC(dbName, outCollName, func(coll *mgo.Collection) error {
            var err error
            var advertiseContents []model.AdvertiseContent
            coll.Find(nil).Skip(i*pageNum).Limit(numPerPage).All(&advertiseContents)
            log.Println("advertiseContents length:")
            log.Println(len(advertiseContents))
            
            /* 这个代码是upsert单行数据
            for j:=0; j<len(advertiseContents); j++ {
                AdvertiseContent := advertiseContents[j]
                advertiseContentValue := advertiseContent.Value
                // advertiseContentValue.Devide = nil
                // advertiseContentValue.CountryCode = nil
                ///advertiseContentValue.Operate = nil
                log.Println("ID_:" + advertiseContent.Id_)
                advertiseContentValue.Id = advertiseContent.Id_
                err := esdb.UpsertType(esIndexName, esAdvertiseContentTypeName, advertiseContent.Id_, advertiseContentValue)
            ...
            */
            if len(advertiseContents) > 0 {
                ...
                for j:=0; j<len(advertiseContents); j++ {
                    ...
                    log.Println(advertiseContent.Id_)
                    log.Println(advertiseContentValue)
                    req := esdb.BulkUpsertTypeDoc(esIndexName, esAdvertiseContentTypeName, advertiseContent.Id_, advertiseContentValue)

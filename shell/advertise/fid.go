...
// Fid 统计计算部分
func FidMapReduct(dbName string, collName string, outCollName string, website_id string) error {
    var err error
    mapStr := `
        function() {  
            ...
            if (fid) {
                ...
                if(this.fid){
                    ...
                    emit(this.fid+"_"+service_date_str+"_"+website_id,{
                        fid: fid,
    ...
    esAdvertiseFidTypeMapping := helper.GetEsAdvertiseFidTypeMapping()
    ...
    //}
    // 初始化mapping
    err = esdb.InitMapping(esIndexName, esAdvertiseFidTypeName, esAdvertiseFidTypeMapping)
    if err != nil {
        return err
    }
    // 同步mongo数据到ES
    ...
    for i:=0; i<pageNum; i++ {
        err = mongodb.MDC(dbName, outCollName, func(coll *mgo.Collection) error {
            var err error
            var advertiseFids []model.AdvertiseFid
            coll.Find(nil).Skip(i*pageNum).Limit(numPerPage).All(&advertiseFids)
            log.Println("advertiseFids length:")
            log.Println(len(advertiseFids))
            
            /* 这个代码是upsert单行数据
            for j:=0; j<len(advertiseFids); j++ {
                AdvertiseFid := advertiseFids[j]
                advertiseFidValue := advertiseFid.Value
                // advertiseFidValue.Devide = nil
                // advertiseFidValue.CountryCode = nil
                ///advertiseFidValue.Operate = nil
                log.Println("ID_:" + advertiseFid.Id_)
                advertiseFidValue.Id = advertiseFid.Id_
                err := esdb.UpsertType(esIndexName, esAdvertiseFidTypeName, advertiseFid.Id_, advertiseFidValue)
            ...
            */
            if len(advertiseFids) > 0 {
                ...
                for j:=0; j<len(advertiseFids); j++ {
                    ...
                    log.Println(advertiseFid.Id_)
                    log.Println(advertiseFidValue)
                    req := esdb.BulkUpsertTypeDoc(esIndexName, esAdvertiseFidTypeName, advertiseFid.Id_, advertiseFidValue)

...
// Campaign 统计计算部分
func CampaignMapReduct(dbName string, collName string, outCollName string, website_id string) error {
    var err error
    mapStr := `
        function() {  
            ...
            if (fec_campaign) {
                ...
                if(fec_campaign){
                    ...
                    emit(fec_campaign+"_"+service_date_str+"_"+website_id,{
                        fid: fid,
    ...
    esAdvertiseCampaignTypeMapping := helper.GetEsAdvertiseCampaignTypeMapping()
    ...
    //}
    // 初始化mapping
    err = esdb.InitMapping(esIndexName, esAdvertiseCampaignTypeName, esAdvertiseCampaignTypeMapping)
    if err != nil {
        return err
    }
    // 同步mongo数据到ES
    ...
    for i:=0; i<pageNum; i++ {
        err = mongodb.MDC(dbName, outCollName, func(coll *mgo.Collection) error {
            var err error
            var advertiseCampaigns []model.AdvertiseCampaign
            coll.Find(nil).Skip(i*pageNum).Limit(numPerPage).All(&advertiseCampaigns)
            log.Println("advertiseCampaigns length:")
            log.Println(len(advertiseCampaigns))
            
            /* 这个代码是upsert单行数据
            for j:=0; j<len(advertiseCampaigns); j++ {
                AdvertiseCampaign := advertiseCampaigns[j]
                advertiseCampaignValue := advertiseCampaign.Value
                // advertiseCampaignValue.Devide = nil
                // advertiseCampaignValue.CountryCode = nil
                ///advertiseCampaignValue.Operate = nil
                log.Println("ID_:" + advertiseCampaign.Id_)
                advertiseCampaignValue.Id = advertiseCampaign.Id_
                err := esdb.UpsertType(esIndexName, esAdvertiseCampaignTypeName, advertiseCampaign.Id_, advertiseCampaignValue)
            ...
            */
            if len(advertiseCampaigns) > 0 {
                ...
                for j:=0; j<len(advertiseCampaigns); j++ {
                    ...
                    log.Println(advertiseCampaign.Id_)
                    log.Println(advertiseCampaignValue)
                    req := esdb.BulkUpsertTypeDoc(esIndexName, esAdvertiseCampaignTypeName, advertiseCampaign.Id_, advertiseCampaignValue)

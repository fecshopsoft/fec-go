...
// Sku Refer 统计计算部分
func SkuReferMapReduct(dbName string, collName string, outCollName string, website_id string) error {
    var err error
    ...
    mapStr := `
        function() {  
            ...
            if (sku) {
                emit(sku+"_"+first_referrer_domain+"_"+service_date_str+"_"+website_id,{
    ...
    esWholeSkuReferTypeMapping := helper.GetEsWholeSkuReferTypeMapping()
    ...
    //}
    // 初始化mapping
    err = esdb.InitMapping(esIndexName, esWholeSkuReferTypeName, esWholeSkuReferTypeMapping)
    if err != nil {
        return err
    }
    // 同步mongo数据到ES
    ...
    for i:=0; i<pageNum; i++ {
        err = mongodb.MDC(dbName, outCollName, func(coll *mgo.Collection) error {
            var err error
            var wholeSkuRefers []model.WholeSkuRefer
            coll.Find(nil).Skip(i*pageNum).Limit(numPerPage).All(&wholeSkuRefers)
            log.Println("wholeSkuRefers length:")
            log.Println(len(wholeSkuRefers))
            
            /* 这个代码是upsert单行数据
            for j:=0; j<len(wholeSkuRefers); j++ {
                wholeBrowser := wholeSkuRefers[j]
            ...
            */
            if len(wholeSkuRefers) > 0 {
                ...
                for j:=0; j<len(wholeSkuRefers); j++ {
                    wholeSkuRefer := wholeSkuRefers[j]
                    wholeSkuReferValue := wholeSkuRefer.Value
                    wholeSkuReferValue.Id = wholeSkuRefer.Id_
                    log.Println("888")
                    log.Println(esIndexName)
                    log.Println(esWholeSkuReferTypeName)
                    log.Println(wholeSkuRefer.Id_)
                    log.Println(wholeSkuReferValue)
                    req := esdb.BulkUpsertTypeDoc(esIndexName, esWholeSkuReferTypeName, wholeSkuRefer.Id_, wholeSkuReferValue)
